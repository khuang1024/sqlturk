...
public class XFDPlus {
	...
	stmt.execute("DROP TABLE IF EXISTS " + Parameters.FD_PLUS_REL_NAME);
	
	// create the FD
	String create = "CREATE TABLE " + Parameters.FD_PLUS_REL_NAME + " AS " + union;
	stmt.execute(create);
	...
	return Parameters.FD_PLUS_REL_NAME;
    }
	...
	while (!inComplete.isEmpty()) {
	    ...
	    // check all other tuples which are connected with is tuple set. Line 7-8
	    for (XTable table : otherTables) {
		for (int i = 0; i < table.size(); i++) {
		    XTuple tuple = table.getTuple(i);
		    if (tupleSet.connects(tuple, dbConn) && (!tupleSet.contains(tuple))) {
			tupleSet.add(tuple);
			// here, we should break, because there can not be
			// two tuples in a single table which are connected with
			// the same tuple set.
			break;
		    }
		}
		...
		for (int i = 0; i < table.size(); i++) {
			...
			XTupleSet prime = getPrime(tuple, tupleSet, dbConn); // Line 11
			...
			if (prime.hasTupleFrom(targetTable)) { // Line 12
				...
				for (int j = 0 ; j < inComplete.size(); i++) { // Line 16
				    ...
				    if (prime.connects(ts, dbConn)) { // Line 17
					ts.addAll(prime);
					inserted = true;
				    }
				}
    ...
    private static XTupleSet getPrime(XTuple tuple, XTupleSet tupleSet, Connection dbConn) throws SQLException {
	XTupleSet prime = new XTupleSet(tuple);
	for (XTuple t : tupleSet.getTuples()) {
	    if (prime.connects(t, dbConn)) {
		prime.add(t);
	    }
	}
	return prime;
